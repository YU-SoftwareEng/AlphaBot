...
### Use Case #9 : 채팅 메시지 저장

**Summary**  
: 인증된 사용자가 대화방 내의 Bot(챗봇)의 메시지를 저장(북마크)하고 카테고리를 설정하여 나중에 다시 볼 수 있도록 하는 기능
...
**Post Conditions**  
: Success – 선택된 메시지가 사용자의 "저장된 메시지"에 저장된다. 시스템이 메시지 저장이 성공했다는 알림을 표시한다.
  Failed – 선택된 메시지가 사용자의 "저장된 메시지"에 저장되지 않는다.

**Main Flow**

s. 사용자가 채팅방의 Bot(챗봇)의 메시지를 저장하려고 한다.
1. 사용자가 채팅방을 연다.
2. 사용자가 저장하려는 Bot(챗봇)의 메시지로 이동한다.
3. 사용자가 메시지의 우측 상단의 저장(북마크) 버튼을 클릭한다.
4. 시스템은 해당 메시지를 저장된 메시지 목록에 저장한다.
5. 시스템은 "북마크에 추가되었습니다."라고 메시지가 성공적으로 저장되었다는 알림을 표시한다.
6. 시스템이 저장 완료 알림을 표시하면 이 use case는 종료된다.

**Extensions**
- 4a. 시스템 오류로 인해 메시지 저장에 실패하는 경우
  → 4a1. 시스템은 "북마크 추가에 실패했습니다."라는 오류 메시지를 표시하고 종료한다.
...
: Success – 선택된 채팅방이 사용자의 채팅목록에서 삭제되고 휴지통으로 이동한다. 시스템이 채팅방 삭제에 성공했다는 알림을 표시한다.
  Failed – 선택된 채팅방이 사용자의 채팅목록에서 삭제되지 않고, 시스템이 채팅방 삭제에 실패했다는 알림을 표시한다.

**Main Flow**
s. 사용자가 채팅목록의 특정 채팅방을 삭제하여 휴지통으로 보내려고 한다.
1. 사용자가 채팅목록 칸에서 삭제하고자 하는 채팅방이 보이도록 이동한다.
2. 사용자는 해당 채팅방의 “삭제”(휴지통 아이콘) 버튼을 클릭한다.
3. 시스템이 "이 채팅방을 휴지통으로 이동하시겠습니까?"라는 확인 팝업을 띄운다. ("확인"과 "취소"버튼이 존재한다.)
4. 사용자가 “확인” 버튼을 클릭한다.
5. 시스템은 해당 채팅방을 채팅목록에서 삭제하고 휴지통으로 이동시킨다.
6. 채팅방이 목록에서 사라지면 이 use case는 종료된다.

**Extensions**
- 4a. 사용자가 “취소” 버튼을 클릭한다.
  → 4a1. 시스템이 해당 팝업을 닫고 채팅방이 삭제되지 않는다.
- 5a. 시스템 오류로 인해 휴지통 이동에 실패하는 경우
  → 5a1. 시스템은 "채팅방을 휴지통으로 이동하지 못했습니다."라는 오류 메시지를 표시하고 종료한다.
...
### Use Case #21 : 검색 기록 조회

**Summary**  
: 인증 사용자가 이전에 검색했던 종목 기록(최근 검색어)을 조회하는 기능
...
**Preconditions**  
: 사용자가 시스템에 로그인된(인증된) 상태여야 한다.

**Trigger**  
: 사용자가 '종목 검색'을 위해 검색창을 선택(클릭/터치)할 때

**Post Conditions**  
: Success – 시스템은 사용자에게 최근 검색 기록 목록을 보여준다.
  Failed – 검색 기록을 불러오지 못하고, 검색 기록이 없다는 메시지를 표시한다.

**Main Flow**

s. 인증 사용자가 검색창을 선택한다.
1. 이 Use case는 인증 사용자가 검색창을 선택할 때 시작된다.
2. 사용자가 검색창을 클릭(터치)한다.
3. 시스템은 해당 사용자의 계정에 저장된 최근 검색 기록을 조회한다.
4. 시스템은 검색창 하단(또는 관련 영역)에 최근 검색 기록 목록을 표시한다.
5. 이 Use case는 검색 기록이 성공적으로 표시되면 끝난다.


**Extensions**
- 3a. 사용자의 검색 기록이 존재하지 않는다.
  → 3a1. 시스템이 "최근 검색 기록이 없습니다."라는 메시지를 표시한다.

**Related**
- Performance: ≤ 1 s 
- Frequency: 사용자당 하루에 평균 5 번
- Concurrency: 제한 없음
- Due Date: 2025. 11. 20

---

### Use Case #22 : 검색 기록 삭제

**Summary**  
: 인증 사용자가 자신의 검색 기록을 개별 또는 전체 삭제하는 기능
...
**Preconditions**  
: 1. 사용자가 시스템에 로그인된 상태여야 한다. 
  2. 'Use case #21: 검색 기록 조회'가 실행되어 검색 기록이 화면에 표시된 상태여야 한다.

**Trigger**  
: 사용자가 검색 기록 목록에서 '개별 삭제(X)' 버튼 또는 '전체 삭제' 버튼을 클릭할 때

**Post Conditions**  
: Success – 1. (개별 삭제) 선택된 검색 기록이 시스템에서 삭제되고 화면의 목록이 갱신된다. 
            2. (전체 삭제) 모든 검색 기록이 시스템에서 삭제되고 화면의 목록이 갱신된다.
  Failed – 검색 기록 삭제에 실패하고, 사용자에게 오류 메시지를 알린다.

**Main Flow**

s. (Scenario A) 인증 사용자가 특정 검색 기록을 삭제한다.
1. 이 Use case는 검색 기록이 표시된 상태에서(Use Case #21) 시작된다.
2. 사용자가 삭제하려는 검색 기록 항목 옆의 '삭제(X)' 버튼을 클릭한다.
3. 시스템은 해당 검색 기록을 데이터베이스에서 삭제한다.
4. 시스템은 화면의 검색 기록 목록을 갱신하여 해당 항목을 제거한다.
5. 이 Use case는 기록이 성공적으로 삭제되면 끝난다.

s. (Scenario B) 인증 사용자가 모든 검색 기록을 삭제한다.
1. 이 Use case는 검색 기록이 표시된 상태에서(Use Case #21) 시작된다.
2. 사용자가 '전체 삭제' 버튼을 클릭한다.
3. 시스템이 "모든 검색 기록을 삭제하시겠습니까?"라는 확인 알림(팝업)을 표시한다.
4. 사용자가 '확인' 버튼을 클릭한다.
5. 시스템은 해당 사용자의 모든 검색 기록을 데이터베이스에서 삭제한다.
6. 시스템은 화면의 검색 기록 목록을 비우고 "최근 검색 기록이 없습니다."라는 메시지를 표시한다.
7. 이 Use case는 모든 기록이 성공적으로 삭제되면 끝난다.


**Extensions**
- 4a. 사용자가 '취소' 버튼을 클릭한다.
  → 4a1. 시스템이 확인 알림을 닫고, Use case는 종료된다.
- (3(A)or5(B))a. 시스템 오류(DB 연결 실패 등)로 인해 삭제에 실패한다.
  → (3(A)or5(B))a1. 시스템이 "삭제에 실패했습니다. 다시 시도해주세요."라는 오류 메시지를 표시한다.

**Related**
- Performance: ≤ 2 s 
- Frequency: 사용자당 주 1-2회
- Concurrency: 제한 없음
- Due Date: 2025. 11. 20
...
**Summary**  
: 주식 투자 정보를 얻기 위해 챗봇과 대화하고 답변을 받는 기능
**Scope / Level**  
: 주식 챗봇 시스템, User level
...
**Preconditions**  
: 투자자가 챗봇 인터페이스에 접속하여 로그인(선택사항)한 상태여야 한다

**Trigger**  
: 투자자가 챗봇 대화 창에 질문을 입력하고 전송하는 경우

**Post Conditions**  
: Success – 챗봇이 질문의 의도를 파악하지 못하거나, 데이터 오류로 인해 답변을 생성하지 못하고 오류 메시지를 반환한다
 Failded – 챗봇이 질문의 의도를 파악하지 못하거나, 데이터 오류로 인해 답변을 생성하지 못하고 오류 메시지를 반환한다

**Main Flow**
1. 투자자가 챗봇과의 대화를 시작한다
2. 투자자가 챗봇 대화 창에 주식 관련 질문(예: "삼성전자 오늘 주가 분석해 줘")을 입력하고 전송한다.
3. 챗봇 시스템은 질문 내용을 분석하고 실시간/과거 데이터 API를 통해 필요한 정보를 조회한다.
4. 챗봇은 조회된 정보를 바탕으로 자연어 처리(NLP) 및 AI 분석 모델을 통해 질문에 대한 답변 텍스트를 생성한다
5. 생성된 답변 텍스트가 투자자에게 표시되며, 이 Use case가 종료된다.

**Extensions**
- 3a.질문에 필요한 실시간 데이터(예: 주가)를 외부 API에서 가져올 수 없는 경우. 챗봇은 "현재 실시간 데이터 연결에 문제가 있습니다. 잠시 후 다시 시도해 주세요."와 같은 오류 메시지를 반환하고 종료된다.
3b.챗봇이 질문의 의도를 명확히 파악하지 못하거나, 답변할 수 없는 범위를 벗어난 질문인 경우. 챗봇은 질문을 재구성하도록 안내하는 메시지(예: "질문을 좀 더 구체적으로 해주시겠어요?")를 반환하고 Step 1로 돌아간다.

**Related**
- Performance: ≤ 3 s
- Frequency: 회원당 주 평균 15회
- Concurrency: 제한 없음
- Due Date: N/A

---

### Use Case #24 : Export Chat(채팅 공유)

**Summary**  
: 챗봇과의 대화 내용 중 일부 또는 전체를 이미지, 텍스트 형태로 외부에 공유하는 기능
**Scope / Level**  
: 주식 챗봇 시스템, User level
...
**Actors**  
: Primary – 챗봇과의 대화내용을 공유하고자 하는 사용자

**Status**  
: Analysis 

**Preconditions**  
: 투자자가 챗봇과의 대화 기록이 남아있는 상태여야 한다.

**Trigger**  
: 투자자가 대화 창에서 '공유' 버튼을 클릭하거나 특정 메시지를 길게 눌러 공유 메뉴를 선택하는 경우

**Post Conditions**  
: Success – 챗봇 대화 내용이 텍스트 또는 이미지 형태로 생성되어 사용자가 지정한 외부 플랫폼(SNS, 메신저 등)으로 성공적으로 전송된다
 Failded – 네트워크 오류 또는 플랫폼 제약으로 인해 공유가 실패하거나, 내용 생성이 원활하지 않아 오류 메시지가 표시된다

**Main Flow**
1. 투자자가 챗봇과의 대화 중 공유하고 싶은 분석 결과 또는 답변을 확인한다
2. 투자자가 공유하고자 하는 메시지 또는 대화 구간을 선택한 후 '공유하기' 버튼을 클릭한다.
3. 챗봇 시스템은 해당 대화 내용을 **공유 가능한 형태(예: 이미지 스크린샷, 포맷된 텍스트)**로 변환하여 임시 저장한다.
4. 투자자가 원하는 외부 플랫폼을 선택하면, 변환된 내용이 해당 플랫폼으로 전송되며 공유가 완료된다
5. 이 Use case는 공유 과정이 완료되고 투자자가 챗봇 화면으로 돌아오면 끝난다

**Extensions**
- 공유하려는 내용에 개인 포트폴리오 정보 등 민감한 데이터가 포함되어 있는 경우.
2a1. 챗봇은 "개인 정보가 포함된 내용은 공유할 수 없습니다." 또는 "개인 정보(예: 잔고, 계좌 번호)는 자동 제외됩니다."라는 경고 메시지를 보여준다.
- 4b.	(플랫폼 전송 실패) 외부 플랫폼으로 내용을 전송하는 과정에서 네트워크 연결에 문제가 발생한다.
4b1. 챗봇은 "네트워크 오류로 공유에 실패했습니다. 다시 시도해 주십시오."라는 메시지를 표시하고, 공유 목록 선택 단계(Step 3)로 돌아간다


**Related**
- Performance: 공유 내용 생성 및 플랫폼 선택 화면 생성 표기 시간≤ 1 s
- Frequency: 투자자당 주 평균 5회
- Concurrency: 제한 없음
- Due Date: N/A

---

### Use Case #25 : Requesting Help / User Guide

**Summary**  
: 챗봇 사용법, 주요 기능 목록, 자주 묻는 질문(FAQ)에 대한 정보를 제공하는 기능
**Scope / Level**  
: 주식 챗봇 시스템, User level
...
**Actors**  
: Primary – 챗봇의 기능 안내가 필요한 사용자

**Status**  
: Analysis 

**Preconditions**  
: 투자자가 챗봇 인터페이스에 접속한 상태여야 한다.

**Trigger**  
: 투자자가 대화 창에 "도움말", "Help", 또는 "사용법"을 입력하거나, 인터페이스의 '도움말' 버튼을 클릭하는 경우

**Post Conditions**  
: Success – 챗봇이 사용자에게 챗봇의 기능 목록, 사용 예시, 또는 FAQ 링크를 포함한 안내 메시지를 명확하게 제시한다
 Failded – 시스템 오류로 인해 안내 메시지를 불러오거나 전송하지 못하고 오류 메시지를 반환한다

**Main Flow**
1. 투자자가 챗봇의 사용 방법이나 기능을 알고자 한다
2. 투자자가 대화 창에 "도움말"을 입력하거나, 지정된 '도움말' 아이콘을 클릭하여 요청한다
3. 챗봇 시스템은 내부에 저장된 '챗봇 사용 가이드' 데이터베이스에서 핵심 기능 목록(예: 주가 조회, 분석 요청, 포트폴리오 분석 등) 및 FAQ를 조회한다
4. 챗봇은 조회된 내용을 정리하여, 사용자가 바로 시도해 볼 수 있는 예시 질문과 기능 목록을 포함한 안내 메시지를 생성한다
5. 생성된 안내 메시지가 투자자에게 표시되며, 이 Use case가 종료된다.

**Extensions**
- (상세 도움 요청) 사용자가 "도움말" 대신 "매매 추천 기능 사용법 알려줘"와 같이 특정 기능에 대한 상세 도움을 요청하는 경우.
2a1. 챗봇은 해당 특정 기능(매매 추천)에 대한 상세한 설명과 구체적인 사용 예시만을 포함하여 안내 메시지를 생성하고 Step 4로 이동한다.
- (내부 데이터 오류) 도움말 콘텐츠를 불러오는 과정에서 시스템 오류가 발생한다.
3b1. 챗봇은 "죄송합니다. 현재 시스템 점검으로 인해 도움말을 제공할 수 없습니다. 잠시 후 다시 시도해 주십시오."라는 메시지를 표시하고 종료한다.


**Related**
- Performance: 안내 메세지 응답 시간≤ 1 s
- Frequency: 투자자당 주 평균 1회
- Concurrency: 제한 없음
- Due Date: N/A
